...
public static class PlaylistApi
{
    public static RouteGroupBuilder MapPlaylistApi(this IEndpointRouteBuilder routes)
    {
        ...
        group.MapPost("/", async (PlaylistCreateDto dto, PlainSightDbContext context, ILoggerFactory loggerFactory) =>
        {
        ...
        group.MapPut("/{id:int}", async (int id, PlaylistCreateDto dto, PlainSightDbContext context, ILoggerFactory loggerFactory) =>
        {
            ILogger logger = loggerFactory.CreateLogger("PlaylistApi");
        ...
        group.MapDelete("/{id:int}", async (int id, PlainSightDbContext context, ILoggerFactory loggerFactory) =>
        {
            ILogger logger = loggerFactory.CreateLogger("PlaylistApi");
            Playlist? playlist = await context.Playlists.FindAsync(id);
            if (playlist == null)
        ...
        group.MapPost("/{playlistId:int}/items", async (int playlistId, AddPlaylistItemDto dto, PlainSightDbContext context, ILoggerFactory loggerFactory) =>
        {
            ILogger logger = loggerFactory.CreateLogger("PlaylistApi");
            Playlist? playlist = await context.Playlists
                .Include(p => p.Items)
        ...
        group.MapDelete("/items/{itemId:int}", async (int itemId, PlainSightDbContext context, ILoggerFactory loggerFactory) =>
        {
            ILogger logger = loggerFactory.CreateLogger("PlaylistApi");
