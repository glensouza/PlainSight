...
# The splash is auto-generated by the PlainSight Player on first run and
# regenerated whenever the player binary version changes (tracked in
# /opt/plainsight/splash.version).
# To use a custom image: drop a 1920x1080 PNG at /opt/plainsight/splash.png
# AND write the current player version to /opt/plainsight/splash.version
# (or any non-empty string that matches future updates) so the regenerator
