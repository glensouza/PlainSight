...
- [x] **[#12](https://github.com/glensouza/PlainSight/issues/12) Remove PlainSight.Player.Photino; add Chromium kiosk player**
  Remove the Photino project (net8.0 pin, niche dependency). Replace its display functionality inside `PlainSight.Player` using an embedded Kestrel HTTP server + `chromium-browser --kiosk`. Port the HTML5 video/playlist page as an embedded resource. Removes the only net8.0 project from the solution.
